...
try {
    ...
    $stmt_folio = $conn->prepare("SELECT folio FROM transferencias WHERE id = ? AND autorizacion_id = ?");
    ...
    foreach ($transferencias_a_rechazar as $solicitud) {
        if ($solicitud['estado'] !== 'Pendiente') {
            throw new Exception("Una de las solicitudes del folio {$folio_a_rechazar} ya ha sido procesada.");
        }
    ...
    $sql_update = 'UPDATE transferencias SET estado = "Rechazado", motivo = ? WHERE folio = ?';
